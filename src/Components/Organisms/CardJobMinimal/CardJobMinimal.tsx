 ...
 * @param {Object} props
 */
const CardJobMinimal = ({ title, subtitle, showArrow = false, titleProps = {}, subtitleProps = {}, className, ...rest }: CardJobMinimalProps): JSX.Element => {
    const classes = useStyles();

    return (
        <div className={clsx(classes.root, 'card-job-minimal')} {...rest}>
            <div className={clsx(classes.cardJobMinimalBody, 'card-job-minimal__body')} style={!showArrow ? { marginRight: 0 } : {}}>
                <div className="card-job-minimal__title-container">
                    <Typography variant="subtitle1" className={clsx(classes.title, 'card-job-minimal__title')} {...titleProps}>
                        {title}
                    </Typography>
                </div>
                <div className="card-job-minimal__location-conatiner">
                    <Typography variant="body2" color="textSecondary" className="card-job-minimal__location" {...subtitleProps}>
                        {subtitle}
                    </Typography>
                </div>
            </div>
            {showArrow ? (
                <div className="card-job-minimal__arrow-container">
                    <Icon fontIconClass="fas fa-angle-right" fontIconColor={colors.grey[700]} className="card-job-minimal__arrow" />
                </div>
            ) : null}
        </div>
    );
