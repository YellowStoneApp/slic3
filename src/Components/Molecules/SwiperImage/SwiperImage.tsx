 ...
 * @param {Object} props
 */
const SwiperImage = ({ items, navigationButtonStyle, imageClassName, className, ...rest }: SwiperImageProps): JSX.Element => {
        ...
        <div className={clsx('swiper-container', 'swiper-image', classes.root, className)} {...rest}>
            <div className="swiper-image__wrapper, swiper-wrapper">
                {items.map((item, index) => (
                    <div className={clsx('swiper-image__slide', 'swiper-slide', classes.swiperSlide)} key={index}>
                        <Image
                            src={item.src}
                            alt={item.alt}
                            srcSet={item.srcSet}
